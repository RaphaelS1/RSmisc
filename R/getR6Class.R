...
#' @param parent numeric, which class should be returned? By default returns the class of the object, increasing
...
#' getR6Class(child$new(), parent = 0)
#' getR6Class(child$new(), parent = 1)
#' getR6Class(parent$new(), FALSE, parent = 0)
#'
#' \dontrun{
...
#'
#' @export
getR6Class <- function(object, classname = TRUE, parent = 0){
  if(classname)
    return(get(class(object)[[parent+1]], envir = parent.env(environment()))$classname)
  else
    return(get(class(object)[[parent+1]], envir = parent.env(environment())))
}
